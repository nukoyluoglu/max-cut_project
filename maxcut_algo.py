        ...
        if len(t_converge) > 0:
            ...
            for energy_hist in energy_hists:
                ...
                for t in range(len(energy_hist)):
                    if np.round(energy_hist[t], 8) == np.round(min_energy_from_entropy, 8):
...
def simulate(structure, system_size, fill, interaction_shape, interaction_radius, algorithm, init_temp, cool_rate, exact_min_energy, exact_min_gap):
    ...
    if exact_min_energy:
        ...
        if abs(min_energy - exact_min_energy) <= exact_min_gap:
            ground_state_found = energy_hist.index(min_energy)
...
def param_search(structure, system_size, fill, interaction_shape, interaction_radius, ensemble, path, exact_min_energy, exact_min_gap, output):
    ...
    output.put(dict(interaction_radius=interaction_radius, init_temp=opt_init_temp, cool_rate=opt_cool_rate, step_from_exact=opt_step_from_exact, step_from_entropy=opt_step_from_entropy, prob_ground_state_per_run=opt_prob_ground_state_per_run, search_runtime=(end - start), exact_min_energy=exact_min_energy))
...
if __name__ == '__main__':
    ...
    output = mp.Queue()
    processes = [mp.Process(target=param_search, args=(structure, system_size, fill, interaction_shape, interaction_radius, ensemble, algo_dir_path, exact_min_energy.get(interaction_radius), exact_min_gap.get(interaction_radius), output)) for interaction_radius in radius_range]
    system_sols = util.parallel_process(processes, output)
