...
def collect_param_stats(temp_hists, energy_hists, ground_states_found, partition_hists, exact_min_energies):
    all_temps_hist = {}
    all_energies_hist = {}
    all_partitions_hist = {}
...
def get_total_iters(stats_vs_t, energy_hists, all_ground_states_found_hist, exact_min_energies, entropy_approx=True):
    ...
    for stat_vs_t in stats_vs_t:
        ...
        t = stat_vs_t['t']
        if exact_min_energies and all_ground_states_found_hist:
...
def simulate(prob, algorithm, init_temp, cool_rate, exact_min_energy, exact_min_gap):
...
def run_trials(structure, system_size, fill, interaction_shape, interaction_radius, algorithm, ensemble, path, init_temp, cool_rate, sample_best_probs):
    param_sols = []
    exact_min_energies = []
    exact_min_gaps = []
    prob = None
    ...
    for i in range(NUM_PARAM_TRIALS):
        ...
        prob.reset()
        param_sols.append(simulate(prob, algorithm, init_temp, cool_rate, exact_min_energy, exact_min_gap))
    ...
    all_temps_hist, all_energies_hist, all_partitions_hist, all_ground_states_found_hist, all_energies_vs_temp, all_partitions_vs_temp, all_ground_states_found_vs_temp = collect_param_stats(temp_hists, energy_hists, ground_states_found, partition_hists, exact_min_energies)
    ...
    min_energy_from_entropy = get_total_iters(stats_vs_t, energy_hists, all_ground_states_found_hist, exact_min_energies)
...
def param_search(structure, system_size, fill, interaction_shape, interaction_radius, ensemble, path):
    ...
    for init_temp, cool_rate in cooling_schedules:
        radius_sols.append(run_trials(structure, system_size, fill, interaction_shape, interaction_radius, algorithm, ensemble, radius_dir_path, init_temp, cool_rate, sample_best_probs))
    ...
    return dict(interaction_radius=interaction_radius, init_temp=opt_init_temp, cool_rate=opt_cool_rate, step_from_exact=opt_step_from_exact, step_from_entropy=opt_step_from_entropy, prob_ground_state_per_run=opt_prob_ground_state_per_run, exact_min_energy=None, search_runtime=(end - start))
...
if __name__ == '__main__':
    ...
    system_sols = [param_search(structure, system_size, fill, interaction_shape, interaction_radius, ensemble, algo_dir_path) for interaction_radius in radius_range]
